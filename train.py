...
def main(
    ...
    # Model params
    dim: int = 512  # Deep-Spin3 suggests either 256 or 512
    n_layers: int = 6  # changed from 8
    n_heads: int = 8  # same as in Deep-Spin3
    vocab_size: int = -1  # defined later by tokenizer
    multiple_of: int = 256  # make SwiGLU hidden layer size multiple of large power of 2
    norm_eps: float = 1e-5

    max_batch_size: int = 32
    max_seq_len: int = 12  # changed from 2048
    ...
    generator = load(
        tokenizer_path, local_rank, world_size, max_seq_len, max_batch_size, ckpt_dir
    )
